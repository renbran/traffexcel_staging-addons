...
class ConstructionDashboard extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            // Restore theme preference — checks in order:
            // 1. localStorage override
            // 2. Odoo's native dark mode (html.o_dark_mode)
            // 3. OS-level prefers-color-scheme
            try {
                const stored = window.localStorage.getItem(THEME_KEY);
                if (stored === "dark" || stored === "light") {
                    this.state.theme = stored;
                } else if (document.documentElement.classList.contains("o_dark_mode")) {
    ...
    async loadData() {
        try {
            ...
            const wip_value = projects
                .filter((p) => p.state === "active" && p.contract_value > 0)
            ...
            const activeProjectCount = projects.filter((p) => p.state === "active").length;
            this.state.active_work_in_progress = activeWOs > 0 ? activeWOs : activeProjectCount;
